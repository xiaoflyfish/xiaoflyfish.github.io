...
addWorker 方法首先是执行了一堆校验，然后使用 `new Worker (firstTask)` 新建了 Worker，最后使用 `t.start ()` 执行 Worker， `t.start () `会执行到 Worker 的 run 方法上
...
2. set (result) 这里是给 outCome 赋值，这样 `Future.get `方法执行时，就可以从 outCome 中拿值。

至此，submit 方法就执行完成了，整体流程比较复杂，我们画一个图释义一下任务提交执行的主流

<img src="https://xiaoflyfish.oss-cn-beijing.aliyuncs.com/image/20210209155252.png" style="zoom:30%;" />
...
线程执行完任务之后，是消亡还是干什么呢？这是一个值得思考的问题，我们可以从源码中找到答案，从 ThreadPoolExecutor 的 runWorker 方法中，不知道有没有同学注意到一个 while 循环
